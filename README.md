net-delta-limit-page.ts

import { Locator, Page } from '@playwright/test'

export class NetDeltaLimitPage {
    inputSwitchLimit: Locator
    inputButterflyLimit: Locator

    constructor(public page: Page) {
        this.inputSwitchLimit = this.page.locator('#bondMultiLegSwitch')
        this.inputButterflyLimit = this.page.locator('#bondMultiLegButterfly')
    }

    async setSwitchNetDeltaLimit(value: string): Promise<void> {
        await this.inputSwitchLimit.clear()
        await this.inputSwitchLimit.fill(value)
    }

    async setButterflyNetDeltaLimit(value: string): Promise<void> {
        await this.inputButterflyLimit.clear()
        await this.inputButterflyLimit.fill(value)
    }
}
...
net-delta-accordion-tests.spec.ts
...
test('Should verify Net Delta red highlight when limit exceeded on Switch RFQ @critical @rfq-flow', async ({
    ...
    await test.step('I configure a low Net Delta limit in User Settings', async () => {
        ...
        await darwinSettings.netDeltaLimitPage.setSwitchNetDeltaLimit('5000')
